		...
		<view :style='{"minHeight":"100vh","width":"100%","padding":"20rpx 0 0 0","position":"relative","background":"#ffffff","height":"100%"}'>
			<view :style='{"padding":"20rpx 0 0 0","alignItems":"center","flex":"1","display":"flex","width":"calc(98% - 120rpx)","position":"relative","justifyContent":"space-between"}'>
				<view :style='{"backgroundColor":"#f5f5f5","margin":"0 30rpx 0 30rpx","color":"#333333","alignItems":"center","borderRadius":"0px","flex":"1","display":"flex","lineHeight":"64rpx","fontSize":"24rpx","height":"64rpx"}'>
					...
					<input v-model="searchForm.dengjimingcheng" type="text" placeholder="等级名称" :style='{"background":"transparent","height":"100%"}'></input>
				</view>
			...
			<view class="uni-product-list" :style='{"padding":"24rpx 24rpx 24rpx 24rpx","margin":"40rpx 0 0 0","alignItems":"flex-start","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"boxShadow":"0 0px 0px #eeeeee","margin":"0 0 40rpx 0","backgroundColor":"#ffffff","borderRadius":"8rpx","flexWrap":"wrap","textAlign":"center","display":"flex","width":"48%","justifyContent":"center","height":"auto"}' v-for="(product,index) in list" :key="index">
					<view class="uni-product-title" :style='{"padding":"0px 20rpx 0px 20rpx","margin":"0 0 0px 0","whiteSpace":"nowrap","color":"#333333","textAlign":"left","overflow":"hidden","borderRadius":"8rpx","width":"100%","lineHeight":"72rpx","fontSize":"28rpx","textOverflow":"ellipsis","order":"2","height":"72rpx"}'>{{product.dengjimingcheng}}</view>
					...
					<view :style='{"padding":"0","margin":"8rpx 0 0 0","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"3"}'>
						<view :style='{"border":"0px solid #dcf8f5","padding":"0px 20rpx 20rpx 20rpx","borderRadius":"60rpx","display":"flex"}' v-if="(userid && isAuth('yingyangdengjibiaozhun','修改')) || (!userid && isAuthFront('yingyangdengjibiaozhun','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"border":"0px solid #f9dede","padding":"0px 20rpx 20rpx 20rpx","borderRadius":"60rpx","display":"flex"}' v-if="(userid && isAuth('yingyangdengjibiaozhun','删除')) || (!userid && isAuthFront('yingyangdengjibiaozhun','删除'))" @click.stop="onDeleteTap(product.id)">
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.dengjimingcheng="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.dengjimingcheng){
					params['dengjimingcheng'] = '%' + this.searchForm.dengjimingcheng + '%'
				}
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`yingyangdengjibiaozhun`, params);
                } else {
                    res = await this.$api.list(`yingyangdengjibiaozhun`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('yingyangdengjibiaozhun', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.dengjimingcheng){
					searchForm['dengjimingcheng'] = '%' + this.searchForm.dengjimingcheng + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`yingyangdengjibiaozhun`, searchForm);
                } else {
                    res = await this.$api.list(`yingyangdengjibiaozhun`, searchForm);
                }
				// 如果是第一页数据置空
